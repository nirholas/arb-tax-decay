# Integrating ArbTaxDecay

## From Solidity

```solidity
import {ArbTaxDecayHook} from "arb-tax-decay/src/hooks/ArbTaxDecayHook.sol";
...
hook.configure(key, ArbTaxDecayHook.Config({ /* see the README */ }));
...
const address = hookAddress("arb-tax-decay", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "ArbTaxDecay"
...
More at https://arb-tax-decay.pages.dev.
